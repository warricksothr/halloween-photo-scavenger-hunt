...
# verdict.
GAME_VERDICTS = {"verified", "obscured", "not_found", "too_small", "misaligned"}
...
def claim(
    ...
    with locked_transaction(request) as writer:
        cur = writer.execute(
            ...
            (ctx.moderator_id, int(time.time()), submission_id, ctx.event_id),
        )
    if cur.rowcount == 0:
...
def verdict(
    ...
    with locked_transaction(request) as writer:
        ...
        log_action(
            ...
            action=Action.VERDICT_ISSUED,
            entity_type="submission",
            entity_id=submission_id,
            details={"verdict": body.verdict, "flavor_text": body.flavor_text},
        )
...
def inappropriate(
    ...
    return {
        "id": submission_id,
        "status": "inappropriate",
        "strike": {"id": strike_id, "level": level, "cooldown_until": cooldown_until},
    }
        ...
        "submissions": [
            {
                "id": s["id"],
                "status": s["status"],
                "riddle": {"text": s["riddle_text"], "sort_order": s["riddle_sort"]},
                "verdict": s["verdict"],
