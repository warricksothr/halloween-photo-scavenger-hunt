...
without blocking the single writer. Migrations keep their own startup
transaction, where the lock does not yet exist.
...
serialize within the single process; reads still run concurrently. The
